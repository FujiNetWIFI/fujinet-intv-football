...
# ---------------------------------------------------------------------------
# Hooked builds: patched original + netcode segment at $6000.
...
$(BUILD)/$(GAME)_lag.bin: $(BUILD)/$(GAME)_patched.asm src/main_lag.asm $(HOOK_SRCS)
	$(AS1600) -o $@ -l $(BUILD)/$(GAME)_lag.lst -s $(BUILD)/$(GAME)_lag.sym src/main_lag.asm
	echo "$$CART_RAM_CFG" >> $(BUILD)/$(GAME)_lag.cfg

# Only the declared patch sites may differ from the original in $5000-$5FFF.
...
# See PORTING.md.   make recon ROM="~/roms/NFL Football.bin"
# ---------------------------------------------------------------------------
ROM ?= $(ROM_ORG)
